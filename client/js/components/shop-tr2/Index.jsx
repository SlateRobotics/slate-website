...
var Component = React.createClass({
  ...
  componentDidMount: function () {
    document.title = "Buy TR2 - Slate Robotics";
    window.scrollTo(0,0);
  },
            ...
            <div className="row" style={{paddingTop:"15px"}}>
              ...
              <div className="col-md-6 col-xs-12" style={{textAlign:"left"}}>
                <div style={{paddingBottom:"25px",borderBottom:"1px solid #ccc"}}>
                  <h1 style={{marginTop:"0px"}}>TR2.config</h1>
                  ...
                  <div style={{
                      ...
                      <div>
                        Orders take 2 to 4 weeks to ship
                      </div>
                  </div>
                </div>
                {this.getConfigItems()}
        ...
        return config.items.map(function (item, j) {
          ...
          if (config.name == "arm" && this.state.reservationToken && item.id == 1) {
            return (
              <ConfigItem
                key={config.name + "-selection-" + j}
                label={"Two-armed TR2 (required by reservation)"}
                value={item.price}
                isSelected={true}
                category={config.name}
                index={item.id}
  ...
  getCheckoutButton: function () {
    if (this.state.order.config[4].value == -1) {
      return (
        <div>
          <div style={{color:"red"}}>
